...
wbegin=`grep "sil_B" $lang/phones.txt | head -1 | awk '{print $2}'`
wend=`grep "#1" $lang/phones.txt | head -1 | awk '{print $2}'`

if [ ! -f $lang/L_align.fst ]; then
  echo "$0: generating $lang/L_align.fst"
  local/make_L_align.sh data/local/lang $lang $lang 2>&1 | tee $dir/log/L_align.log
fi

$cmd $dir/log/align_to_words.log \
  ali-to-phones $dir/final.mdl "ark:gunzip -c $dir/ali.*.gz|" ark,t:- \| \
  phones-to-prons $lang/L_align.fst $wbegin $wend ark:- "ark,s:utils/sym2int.pl -f 2- $lang/words.txt <$data/test/text|" ark,t:- \| \
